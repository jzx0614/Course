...
namespace C09102014
{
	...
	/// </summary>
	public class select_product : System.Web.UI.Page
	{
		...
		{
			string sqlcmd="select product.*,類別名稱 from product,class where product.類別編號=class.類別編號";		
			if(TextBox1.Text!="")
				sqlcmd +=" and 拍賣編號 ='"+TextBox1.Text+"'";
			if(TextBox2.Text!="")
				sqlcmd +=" and 商品名稱 LIKE '%"+TextBox2.Text+"%'";
			if(TextBox3.Text!="")
				sqlcmd +=" and 商品標題 LIKE '%"+TextBox3.Text+"%'";
			if(TextBox4.Text!="")
				sqlcmd +=" and 商品數量 LIKE '%"+TextBox4.Text+"%'";
			BindGrid(sqlcmd);
		}
