...
namespace C09102014
{
	...
	/// WebForm1 的摘要描述。
	/// </summary>
	public class update_product : System.Web.UI.Page
	{
		...
		{
			SqlDataAdapter myCommand1 = new SqlDataAdapter("select product.*,類別名稱 from product,class where product.類別編號=class.類別編號", myConnection);		
		...
		{
			string updateCmd = "UPDATE product SET  商品名稱 = @Pname, 商品標題 = @Ptitle, 商品數量 = @Pqty, ";
			updateCmd+= "商品圖片 = @Ppic, 類別編號 = @class, 商品敘述=@desc where  拍賣編號 = @Id";
			...
			myCommand.Parameters.Add(new SqlParameter("@class",SqlDbType.Int,4));
			myCommand.Parameters.Add(new SqlParameter("@desc",SqlDbType.VarChar,200));
			myCommand.Parameters["@Id"].Value = MyDataGrid.DataKeys[(int)e.Item.ItemIndex];

			string[] cols = {"@Id","@Pname","@Ptitle","@Pqty","@Ppic","@class","@desc"};
