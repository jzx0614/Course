...
{  //==========================create table===========================
    ...
    regtable[0].SetValue("A","0");    regtable[1].SetValue("X","1");
    regtable[2].SetValue("L","2");    regtable[3].SetValue("PC","8");
    regtable[4].SetValue("SW","9");   regtable[5].SetValue("B","3");
    regtable[6].SetValue("S","4");    regtable[7].SetValue("T","5");
    regtable[8].SetValue("F","6");
    
}
int ModeTable::search(char *temp)
{   char buff[10];
    int flag=0;
    if(temp[0]=='+')flag=1;
    strcpy(buff,temp+flag);
    for(int i=0;i<59;i++)
    {
...
char* ModeTable::base16(char *buff,int N)
{
    char temp[]={"0123456789ABCDEF"};
    
    for(int i=0,j=3,t=0;i<N;i++)
    {
...
void ModeTable::indication_2(char *buff)
{
    char temp[2][3];
    char mcode_2[17],hex[5]; 
    ...
    strcpy(temp[1],strstr(buff,",")+1);
    for(int j=0;j<2;j++)
    for(int i=0;i<9;i++)
    {
        if (!strcmp(temp[j],regtable[i].GetMnem()))
        {
                //=============base16 to base2===========             
                base2(*regtable[i].Getnum());
                strcpy(re+(4*j),base2buff);
                //=============return format=============
        }
    }
    strcpy(mcode_2,op);
...
void ModeTable::indication_4(char *buff)
{
   char temp[]={"nixbpe"};
