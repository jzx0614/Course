...
aa= 0:0.2:0.8;
plot(aa,a_spann,'k^-');        hold on
plot(aa,spann,'msquare--');    hold on    
plot(aa,msc_node_weight,'g>-');hold on
%plot(aa,msc_water,'yo-.');     hold on
plot(aa,node_weight,'r*:');    hold on
%plot(aa,water,'bx-.');         hold on
...
xlabel('Number of critical grid');
%xlabel('Group-prob');
ylabel('Number of sensor');

for i = 1 : length(node_weight)
%    text(25*(i+1)+1,node_weight(i),[num2str(100*(a_spann(i)-node_weight(i))/node_weight(i)) '%']) ;
%    text(25*(i+1)+1,water(i),[num2str(100*(a_spann(i)-water(i))/water(i)) '%']) ;
%    text(25*(i+1)+1,spann(i),[num2str(100*(a_spann(i)-spann(i))/spann(i)) '%']) ;
%    text(25*(i+1)+1,msc_node_weight(i),[num2str(100*(a_spann(i)-msc_node_weight(i))/msc_node_weight(i)) '% msc-mw']) ;
%    text(25*(i+1)+1,msc_water(i),[num2str(100*(a_spann(i)-msc_water(i))/msc_water(i)) '%']) ;

%   text(25*(i+1)+1,water(i),[num2str(100*(node_weight(i)-water(i))/water(i)) '%']) ;
%   text(25*(i+1)+1,msc_water(i),[num2str(100*(msc_node_weight(i)-msc_water(i))/msc_water(i)) '%']) ;
end
