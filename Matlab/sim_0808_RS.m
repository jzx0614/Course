...
for rs_group = 1 : 5
    for map_id = 1 : map_avg_num
        filename = [testcase '\node_weight\' num2str(rs_group) '_RS\' num2str(map_id) '_1_map_deployment.txt'];
        fid = fopen(filename);
        fgetl(fid);fgetl(fid);
        node_weight_all(index)=fscanf(fid,'sensor num= %d');
        fclose(fid);

        filename_2 = [testcase '\spann\' num2str(rs_group) '_RS\' num2str(map_id) '_1_map_deployment.txt'];
        fid_2 = fopen(filename_2);
        fgetl(fid_2);fgetl(fid_2);
        spann_all(index)=fscanf(fid_2,'sensor num= %d');
        fclose(fid_2); 

        filename_3 = [testcase '\water\' num2str(rs_group) '_RS\' num2str(map_id) '_1_map_deployment.txt'];
        fid_3 = fopen(filename_3);    
        fgetl(fid_3);fgetl(fid_3);
        water_all(index)=fscanf(fid_3,'sensor num= %d');
        fclose(fid_3);         

        filename_4 = [testcase '\all_spann\' num2str(rs_group) '_RS\' num2str(map_id) '_1_map_deployment.txt'];
        fid_4 = fopen(filename_4);
        fgetl(fid_4);fgetl(fid_4);
        a_spann_all(index)=fscanf(fid_4,'sensor num= %d');
        fclose(fid_4); 
      
        filename_5 = [testcase '\msc_node_weight\' num2str(rs_group) '_RS\' num2str(map_id) '_1_map_deployment.txt'];
        fid_5 = fopen(filename_5);
        fgetl(fid_5);fgetl(fid_5);
        msc_node_weight_all(index)=fscanf(fid_5,'sensor num= %d');
        fclose(fid_5);

        filename_6 = [testcase '\msc_water\' num2str(rs_group) '_RS\' num2str(map_id) '_1_map_deployment.txt'];
...
for i = 1 : length(node_weight)
   ...
   text(i+0.1,spann(i),[num2str(100*(a_spann(i)-spann(i))/spann(i)) '%']) ;
   text(i+0.1,msc_node_weight(i)-2,[num2str(100*(a_spann(i)-msc_node_weight(i))/msc_node_weight(i)) '% ']) ;
